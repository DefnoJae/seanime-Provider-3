...
function detectStreamBatch(s: TorrentioStream): boolean {
    const name = firstLine(s.title) || (s.behaviorHints && s.behaviorHints.filename) || s.name || ""
    if (detectBatch(name)) return true
    if (/\bseason\s+\d{1,2}\b/i.test(name)) return true
    // A standalone season token usually denotes a season pack. Do not match
    // normal episode tokens such as S01E05.
    return /\bS\d{1,2}(?!E\d)\b/i.test(name)
}
...
function parseSeasonRange(title: string): { start: number; end: number } | null {
    ...
    const single = t.match(/\bS\d{1,2}\b/i) || t.match(/\bSeason\s+\d{1,2}\b/i)
    if (single) {
...
class Provider {
    ...
    private async resolveViaTmdb(media: Media): Promise<ResolvedIds> {
        ...
        const type = isMovie ? "movie" : "tv"
        const seasonYear = media.startDate && media.startDate.year ? media.startDate.year : 0
        for (let i = 0; i < unique.length; i++) {
            ...
            if (seasonYear && !isMovie) {
                url += "&first_air_date_year=" + seasonYear
            }
            if (isMovie && seasonYear) {
                url += "&year=" + seasonYear
            }
            try {
    ...
    private async fetchForMedia(ids: ResolvedIds, media: Media, ep: number): Promise<TorrentioStream[]> {
        ...
        const requests: Array<Promise<TorrentioStream[]>> = []

        if (ids.kitsuId) {
            if (movieOrSingle) {
                requests.push(this.fetchStreams(this.movieUrl(ids.kitsuId)))
                requests.push(this.fetchStreams(this.seriesUrl(ids.kitsuId, 1)))
            } else {
                requests.push(this.fetchStreams(this.seriesUrl(ids.kitsuId, ep)))
            }
        }

        if (ids.imdbId) {
            if (movieOrSingle) {
                requests.push(this.fetchStreams(this.imdbMovieUrl(ids.imdbId)))
                requests.push(this.fetchStreams(this.imdbSeriesUrl(ids.imdbId, 1, 1)))
            } else {
                const season = (ids.tmdbSeason && ids.tmdbSeason > 0) ? ids.tmdbSeason : 1
                requests.push(this.fetchStreams(this.imdbSeriesUrl(ids.imdbId, season, ep)))
            }
        }

        if (requests.length === 0) return []
        const results = await Promise.all(requests)
        const combined: TorrentioStream[] = []
        for (let i = 0; i < results.length; i++) {
            for (let j = 0; j < results[i].length; j++) combined.push(results[i][j])
        }
        return combined
    }
